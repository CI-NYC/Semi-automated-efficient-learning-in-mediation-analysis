...
class Learner(nn.Module):
    ...
    def __init__(self, n_t, n_hidden, p, degree, interaction_only=False):
        ...
        self.common1 = nn.Sequential(nn.Dropout(p=p), nn.Linear(n_t - 4, n_common), nn.ReLU(),
                                    ...
                                    nn.Dropout(p=p), nn.Linear(n_common, n_common), nn.ReLU())
        self.common2 = nn.Sequential(nn.Dropout(p=p), nn.Linear(n_t - 2, n_common), nn.ReLU(),
                                    nn.Dropout(p=p), nn.Linear(n_common, n_common), nn.ReLU(),
                                    nn.Dropout(p=p), nn.Linear(n_common, n_common), nn.ReLU())
        self.common3 = nn.Sequential(nn.Dropout(p=p), nn.Linear(n_t - 0, n_common), nn.ReLU(),
                                    nn.Dropout(p=p), nn.Linear(n_common, n_common), nn.ReLU(),
                                    nn.Dropout(p=p), nn.Linear(n_common, n_common), nn.ReLU())
    ...
    def forward(self, x):
        ...
        return torch.cat([riesz1, riesz2, riesz3], dim=1)

moment_fn_1 = recant_nonprime_moment_fn_1
moment_fn_2 = recant_nonprime_moment_fn_2
moment_fn_3 = recant_nonprime_moment_fn_3
...
agmm = RieszNetNonPrime(learner, moment_fn_1, moment_fn_2, moment_fn_3, a_1 = 0, a_2 = 1, a_3 = 0)
...
alpha_results = pd.DataFrame(data={"A": A, "Z_1": Z_1, "Z_2": Z_2, "M_1": M_1, "M_2": M_2, "W_1": W_1, "W_2": W_2, "W_3": W_3, "Y": Y,  "alpha1": agmm.predict(X)[:,0], "alpha2": agmm.predict(X)[:,1], "alpha3": agmm.predict(X)[:,2], "label": int(label)})

alpha_results.to_csv("/gpfs/home/ll4245/Projects/RieszLearning/results_010/N_%d_seq_%d.csv" % (N, int(label)))
