...
from utils.riesznet import RieszNetNDE
...
class Learner(nn.Module):
    ...
    def __init__(self, n_t, n_hidden, p, degree, interaction_only=False):
        ...
        self.common1 = nn.Sequential(nn.Dropout(p=p), nn.Linear(n_t - 2, n_common), nn.ReLU(),
                                    nn.Dropout(p=p), nn.Linear(n_common, n_common), nn.ReLU(),
                                    nn.Dropout(p=p), nn.Linear(n_common, n_common), nn.ReLU())
        self.common2 = nn.Sequential(nn.Dropout(p=p), nn.Linear(n_t - 0, n_common), nn.ReLU(),
                                    nn.Dropout(p=p), nn.Linear(n_common, n_common), nn.ReLU(),
                                    nn.Dropout(p=p), nn.Linear(n_common, n_common), nn.ReLU())
    ...
    def forward(self, x):
        ...
        return torch.cat([riesz1, riesz2], dim=1)
...
filename = "~/Projects/RieszLearning/make_data_nonnull_NDENIE_" + str(N) + "/simulation_test_all_batches_" + str(N) + "_cont_multi_Z_M_Y_recantingtwins_array_" + str(label) + ".csv"
...
agmm = RieszNetNDE(learner, moment_fn_1, moment_fn_2, a_1 = 1, a_2 = 0)
...
alpha_results = pd.DataFrame(data={"A": A, "Z_1": Z_1, "Z_2": Z_2, "M_1": M_1, "M_2": M_2, "W_1": W_1, "W_2": W_2, "W_3": W_3, "Y": Y,  "alpha1": agmm.predict(X)[:,0], "alpha2": agmm.predict(X)[:,1], "label": int(label)})

alpha_results.to_csv("/gpfs/home/ll4245/Projects/RieszLearning/results_10/N_%d_seq_%d.csv" % (N, int(label)))
