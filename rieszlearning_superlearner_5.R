# Num: 5
# Intermediate Confounder in the recanting twins
...
filename = paste0("~/Projects/RieszLearning/results_011/N_", N, "_seq_", label, ".csv")
...
filename = paste0("~/Projects/RieszLearning/results_0111/N_", N, "_seq_", label, ".csv")
combined_alpha_results_RDE_RIE_cont_M_cont_Z_a_10 <- read_csv(filename)
...
esti4 <- alpha_3 * (Y - theta_3) +
  alpha_2 * (b_3 - theta_2) + 
  alpha_1 * (b_2 - theta_1) + b_1

res = mean(esti1 - esti2 + esti3 - esti4)
res_upper = res + qnorm(0.975) * sd(esti1 - esti2 + esti3 - esti4) / sqrt(N)
res_lower = res - qnorm(0.975) * sd(esti1 - esti2 + esti3 - esti4) / sqrt(N)

data = data.frame(res = res, res_upper = res_upper, res_lower = res_lower)

save(data, file=paste0("/gpfs/home/ll4245/Projects/RieszLearning/superlearner_5/", as.integer(Sys.getenv('SLURM_ARRAY_TASK_ID')),".RData"))